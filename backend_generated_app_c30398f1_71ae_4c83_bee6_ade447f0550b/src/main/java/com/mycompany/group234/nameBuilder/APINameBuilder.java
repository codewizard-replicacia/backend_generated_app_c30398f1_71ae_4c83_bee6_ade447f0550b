...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "Bus".equals(entityTypeName) ? "Buses" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Trip".equals(entityTypeName) ? "Trips" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Driver".equals(entityTypeName) ? "Drivers" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Route".equals(entityTypeName) ? "Routes" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
